...
function addPaymentDateToRow(row, paymentDate) {
  ...
  if (existingDate) {
    existingDate.textContent = `Pagado: ${paymentDate}`;
...
function removeExtraPaymentLists(modalRoot) {
  modalRoot.querySelectorAll(
    '[data-payment-history-section], [data-consumos-payment-history], [data-consumos-last-payment]'
...
async function patchVisibleAccountModal() {
  ...
  removeExtraPaymentLists(modalRoot);
