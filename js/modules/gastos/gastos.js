...
function render() {
  ...
  root.innerHTML = `<section class="p-4 md:p-6 space-y-5">
    ...
    <div class="bg-white border rounded-xl overflow-hidden"><div class="p-4 border-b"><h3 class="font-bold">Obligaciones registradas</h3><p class="text-xs text-slate-500 mt-1">Puedes realizar varios abonos hasta completar el valor total de cada cuenta.</p></div><div class="overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-slate-50"><tr><th class="p-3 text-left">Fecha / vence</th><th class="p-3 text-left">Descripción</th><th class="p-3 text-left">Estado</th><th class="p-3 text-right">Total</th><th class="p-3 text-right">Abonado</th><th class="p-3 text-right">Saldo</th><th class="p-3 text-right">Acciones</th></tr></thead><tbody>${state.expenses.map(expense => {
...
function showPaymentModal(expenseId, balance) {
  ...
  card.innerHTML = `<form id="payment-form">
    <div class="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-5 text-white"><div class="flex items-start justify-between gap-4"><div><p class="text-xs font-semibold uppercase tracking-wider text-emerald-100">Abono a cuenta por pagar</p><h3 class="text-xl font-bold mt-1">Registrar abono o pago total</h3><p class="text-sm text-emerald-50 mt-1">${escapeHtml(expense?.description || 'Gasto registrado')}</p></div><button type="button" data-close-modal class="text-2xl leading-none text-white/80 hover:text-white" aria-label="Cerrar">×</button></div></div>
    <div class="p-6 space-y-5">
      ...
      <div class="rounded-xl bg-slate-50 border border-slate-200 p-4 flex justify-between items-center"><span class="text-sm text-slate-600">Saldo que quedará después del abono</span><strong id="remaining-balance" class="text-lg text-slate-800">${money(0)}</strong></div><p id="modal-feedback" class="hidden text-sm rounded-lg p-3"></p>
    </div>
    <div class="bg-slate-50 border-t px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-2"><button type="button" data-close-modal class="px-5 py-2.5 rounded-lg border border-slate-300 font-semibold text-slate-700 hover:bg-white">Cancelar</button><button type="submit" class="px-5 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 font-semibold text-white shadow-sm">Registrar abono</button></div>
  </form>`;
  ...
  const remainingBalance = card.querySelector('#remaining-balance');
  const refreshRemaining = () => { remainingBalance.textContent = money(Math.max(0, balance - Number(amountInput.value || 0))); };
  amountInput.addEventListener('input', refreshRemaining);
  ...
  form.addEventListener('submit', async event => {
    ...
    if (error) { modalFeedback.textContent = error.message; modalFeedback.className = 'text-sm rounded-lg p-3 bg-red-50 text-red-700'; submit.disabled = false; submit.textContent = 'Registrar abono'; return; }
