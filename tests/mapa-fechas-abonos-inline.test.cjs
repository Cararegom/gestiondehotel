...
test('Ver consumos elimina listas duplicadas de historial y deja solo la lista principal', () => {
  assert.match(enhancer, /removeExtraPaymentLists/);
  assert.match(enhancer, /data-payment-history-section/);
  assert.match(enhancer, /data-consumos-payment-history/);
  assert.match(enhancer, /data-consumos-last-payment/);
  assert.doesNotMatch(enhancer, /Historial de pagos y abonos/);
  assert.doesNotMatch(enhancer, /renderPaymentHistory/);
});

test('el parche reintenta si el modal o los globals aun no estan disponibles', () => {
  assert.match(enhancer, /setInterval/);
  assert.match(enhancer, /POLL_MS/);
  assert.match(enhancer, /RETRY_MS/);
  assert.match(enhancer, /window\.supabase/);
  assert.match(enhancer, /window\.hotelIdGlobal/);
});
