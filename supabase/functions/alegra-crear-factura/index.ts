...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const facturaData = typeof payload?.facturaData === 'object' && payload?.facturaData ? payload.facturaData as Record<string, unknown> : {};
    ...
    const contactoNombre = sanitizeString((facturaData?.cliente as Record<string, unknown>)?.nombre, 120) || 'Cliente de prueba';
    const contactoEmail = sanitizeString((facturaData?.cliente as Record<string, unknown>)?.email, 120);
    const contactoIdentificacion = sanitizeString(
      (facturaData?.cliente as Record<string, unknown>)?.identificacion,
      40,
    );
    ...
    return jsonResponse(
      {
        ok: true,
        message:
          'Conexion con Alegra validada y payload de factura de prueba recibido. La creacion automatica completa queda lista para la siguiente fase de integracion.',
        facturaId: null,
        payloadResumen: {
    ...
    console.error('Error inesperado en alegra-crear-factura:', error);
