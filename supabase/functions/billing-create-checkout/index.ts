...
function sanitizeString(value: unknown, maxLength = 140) {
  ...
  if (typeof value === 'bigint') return value.toString().slice(0, maxLength);
  return '';
}

function normalizeRole(value: unknown) {
  return sanitizeString(value, 60).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}
...
function buildCheckoutReference({ paymentType, billingPeriod, hotelId, planId }: {
  paymentType: string;
...
function getPromoBienvenidaStatus(hotel: Record<string, unknown>, pagos: Array<Record<string, unknown>> = []) {
  ...
  const vigentePorTiempo = Boolean(
    promoWindowEnd && !Number.isNaN(promoWindowEnd.getTime()) && Date.now() <= promoWindowEnd.getTime()
  );
  const pagosUsados = Math.min(PROMO_BIENVENIDA_MESES, getPositiveSubscriptionPayments(pagos).length);
...
function resolvePlanUsd(plan: Record<string, unknown> | null | undefined) {
  const key = String(plan?.nombre ?? '').trim().toLowerCase();
  return USD_PRICES[key] ?? 0;
}
...
function safeReturnUrl(value: unknown) {
  const fallback = 'https://gestiondehotel.com/app/index.html#/micuenta';
  const candidate = sanitizeString(value, 500);
...
async function createMercadoPagoCheckout({ amountUSD, planName, planId, paymentType, billingPeriod, hotelId, userEmail, currentUrl }: {
  amountUSD: number;
  planName: string;
  planId: string;
  paymentType: string;
  ...
  if (!MERCADOPAGO_ACCESS_TOKEN) throw new Error('MERCADOPAGO_ACCESS_TOKEN is not configured.');
  const externalReference = buildCheckoutReference({ paymentType, billingPeriod, hotelId, planId });
  const response = await fetch('https://api.mercadopago.com/checkout/preferences', {
    ...
    body: JSON.stringify({
      items: [{
        ...
        quantity: 1,
        currency_id: 'USD',
        unit_price: Number.parseFloat(amountUSD.toFixed(2))
      }],
      ...
      back_urls: { success: currentUrl, pending: currentUrl, failure: currentUrl }
    })
  });
  ...
  return { provider: 'mercadopago', checkoutUrl: data.init_point as string, externalReference };
}

function createWompiCheckout({ amountCOP, planId, paymentType, billingPeriod, hotelId, userEmail }: {
  amountCOP: number;
  planId: string;
  paymentType: string;
  billingPeriod: string;
  ...
  if (!WOMPI_PUBLIC_KEY) throw new Error('WOMPI_PUBLIC_KEY is not configured.');
  const reference = buildCheckoutReference({ paymentType, billingPeriod, hotelId, planId });
  ...
  return { provider: 'wompi', checkoutUrl, externalReference: reference };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const currentUrl = safeReturnUrl(payload?.currentUrl);
    ...
    if (!token) return jsonResponse({ error: 'Authentication required.' }, 401, origin);

    const { data: authData, error: authError } = await supabaseAdmin.auth.getUser(token);
    const authUser = authData?.user;
    if (authError || !authUser) {
      console.warn('billing-create-checkout: token invalido o expirado.', authError?.message);
    ...
    if (profileError) {
      console.error('Error cargando perfil para checkout:', profileError);
      return jsonResponse({ error: 'No se pudo verificar el usuario.' }, 500, origin);
    }
    if (hotelError) {
      console.error('Error cargando hotel en billing-create-checkout:', hotelError);
      return jsonResponse({ error: 'No se pudo cargar el hotel.' }, 500, origin);
    }
    if (!hotel) return jsonResponse({ error: 'Hotel not found.' }, 404, origin);

    const belongsToHotel = String(profile?.hotel_id || '') === String(hotelId);
    const roleCanManage = ADMIN_ROLES.has(normalizeRole(profile?.rol));
    const isCreator = String(hotel.creado_por || '') === String(authUser.id);
    if (!(isCreator || (belongsToHotel && roleCanManage))) {
      console.warn('billing-create-checkout: intento no autorizado.', {
        actor: authUser.id,
        requestedHotelId: hotelId,
        profileHotelId: profile?.hotel_id || null,
    ...
    const selectedPlan = plans.find((plan) => String(plan.id) === planId) ?? (paymentType === 'renew' ? currentPlan : null);
    if (!selectedPlan) return jsonResponse({ error: 'Selected plan not found.' }, 404, origin);
    ...
    if (paymentType === 'upgrade') {
      ...
      let fechaInicioCiclo = new Date(String(hotel.suscripcion_inicio ?? hotel.trial_inicio ?? hotel.creado_en ?? ''));
      ...
      const diasRestantesSeguro = Math.max(0, Math.ceil((fechaFinCiclo.getTime() - Date.now()) / (1000 * 60 * 60 * 24)));

      const creditoNoUsadoCOP = (Number(currentPlan?.precio_mensual ?? 0) / diasCicloSeguro) * diasRestantesSeguro;
      const costoNuevoRestanteCOP = (Number(selectedPlan?.precio_mensual ?? 0) / diasCicloSeguro) * diasRestantesSeguro;
      const creditoNoUsadoUSD = (resolvePlanUsd(currentPlan) / diasCicloSeguro) * diasRestantesSeguro;
      const costoNuevoRestanteUSD = (resolvePlanUsd(selectedPlan) / diasCicloSeguro) * diasRestantesSeguro;
      ...
      const promo = applyPromoBienvenida({ ...baseAmounts, periodo: billingPeriod, promoStatus });
    ...
    const userEmail = sanitizeString(authUser.email, 120);
    if (!userEmail) return jsonResponse({ error: 'The authenticated account has no valid email.' }, 400, origin);
    ...
    const checkout = currency === 'USD'
      ...
      : createWompiCheckout({ amountCOP: finalCOP, planId, paymentType, billingPeriod, hotelId, userEmail });
