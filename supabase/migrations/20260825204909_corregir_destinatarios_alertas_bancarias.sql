...
BEGIN
  ...
  WHERE u.hotel_id = v_event.hotel_id
    ...
    AND lower(COALESCE(u.rol::text, '')) IN (
      'usuario', 'recepcionista', 'recepcion', 'admin', 'administrador'
    )
  ON CONFLICT (hotel_id, usuario_id, entidad_tipo, entidad_id)
    WHERE entidad_tipo = 'bank_payment_event' AND usuario_id IS NOT NULL
  DO UPDATE SET mensaje = EXCLUDED.mensaje, leida = false, actualizado_en = now(),
