-- Recupera y versiona en produccion el rol operativo que ya existia en staging.
-- Objetivo: permitir crear usuarios de Mantenimiento / Conserje desde Usuarios
-- y mantener el acceso operativo limitado mediante helpers dedicados.

CREATE OR REPLACE FUNCTION public.es_nombre_rol_mantenimiento_conserje(p_nombre text)
RETURNS boolean LANGUAGE sql IMMUTABLE SECURITY INVOKER SET search_path TO 'public'
AS $$ SELECT lower(trim(coalesce(p_nombre,'')))='mantenimiento' OR lower(trim(coalesce(p_nombre,''))) LIKE '%mantenimiento%' OR lower(trim(coalesce(p_nombre,''))) LIKE '%conserje%'; $$;
...
VALUES('Mantenimiento / Conserje','Personal operativo de mantenimiento: mapa de habitaciones solo lectura, mantenimiento y control de energia.')
      ...
      jsonb_build_object(
        ...
        'estado',CASE
          WHEN r.id IS NOT NULL
            ...
            AND r.fecha_fin<=now() THEN 'tiempo agotado'
