...
create table if not exists public.tarifas_programadas_habitacion (
  ...
  constraint tarifas_programadas_modalidad_check check (modalidad in ('noche','tiempo_estancia')),
  constraint tarifas_programadas_dias_check check (
    dias_semana <@ array[0,1,2,3,4,5,6]::integer[]
  ),
  constraint tarifas_programadas_fechas_check check (
    fecha_fin is null or fecha_inicio is null or fecha_fin >= fecha_inicio
  ),
  constraint tarifas_programadas_scope_check check (
    (modalidad = 'noche' and tiempo_estancia_id is null)
    or (modalidad = 'tiempo_estancia' and tiempo_estancia_id is not null)
  ),
  constraint tarifas_programadas_precio_check check (
...
create index if not exists idx_tarifas_programadas_hotel_activo
  on public.tarifas_programadas_habitacion(hotel_id, activo);
...
grant select, insert, update, delete on public.tarifas_programadas_habitacion to authenticated;
...
to authenticated
using (
  hotel_id = (select u.hotel_id from public.usuarios u where u.id = auth.uid())
);
