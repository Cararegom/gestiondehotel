...
-- NO volver a ejecutar para sincronizar historial; conserva el estado productivo observado.

CREATE OR REPLACE FUNCTION public.normalizar_producto_transferencia(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
AS $function$
  SELECT regexp_replace(
    public.normalizar_texto_transferencia(p_value),
    '[^a-z0-9]+',
    '',
    'g'
  );
...
CREATE OR REPLACE FUNCTION public.transferir_tienda_a_terraza(p_producto_tienda_id uuid, p_cantidad integer, p_usuario_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  v_nombre_key := public.normalizar_producto_transferencia(v_tienda.nombre);
  v_permite_michelada :=
    public.normalizar_texto_transferencia(COALESCE(v_tienda.categoria_nombre, '')) LIKE '%cerve%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.categoria_nombre, '')) LIKE '%beer%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%cerve%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%beer%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%corona%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%aguila%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%poker%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%club colombia%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%costena%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%heineken%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%budweiser%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%stella%'
    OR public.normalizar_texto_transferencia(COALESCE(v_tienda.nombre, '')) LIKE '%modelo%';
   ...
   WHERE hotel_id = v_tienda.hotel_id
     AND (
       ...
       OR (
         ...
         AND v_tienda.codigo_barras IS NOT NULL
         AND codigo_barras = v_tienda.codigo_barras
       )
       OR public.normalizar_producto_transferencia(nombre) = v_nombre_key
     )
   ORDER BY
     CASE
       WHEN tienda_producto_id = p_producto_tienda_id THEN 1
       WHEN codigo_barras IS NOT NULL AND v_tienda.codigo_barras IS NOT NULL AND codigo_barras = v_tienda.codigo_barras THEN 2
       WHEN public.normalizar_producto_transferencia(nombre) = v_nombre_key THEN 3
       ELSE 4
     END,
     activo DESC,
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'cantidad', p_cantidad,
      'enlazado_a_existente', v_terraza.id IS NOT NULL
    )
  );
...
CREATE OR REPLACE FUNCTION public.transferir_terraza_a_tienda(p_producto_terraza_id uuid, p_cantidad integer, p_usuario_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_hotel_terraza constant uuid := '38373fa5-b953-4aa9-b4e9-25b9739be5f2'::uuid;
  v_nombre_key text;
  v_enlazado_a_existente boolean := false;
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Debes estar autenticado para transferir inventario.';
  END IF;
  ...
  v_nombre_key := public.normalizar_producto_transferencia(v_terraza.nombre);
  ...
  SELECT *
   ...
   WHERE hotel_id = v_terraza.hotel_id
     AND (
       ...
       OR (
         codigo_barras IS NOT NULL
         AND v_terraza.codigo_barras IS NOT NULL
         AND codigo_barras = v_terraza.codigo_barras
       )
       OR public.normalizar_producto_transferencia(nombre) = v_nombre_key
     )
   ORDER BY
     CASE
       WHEN id = v_terraza.tienda_producto_id THEN 1
       WHEN codigo_barras IS NOT NULL AND v_terraza.codigo_barras IS NOT NULL AND codigo_barras = v_terraza.codigo_barras THEN 2
       WHEN public.normalizar_producto_transferencia(nombre) = v_nombre_key THEN 3
       ELSE 4
     END,
     activo DESC,
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'cantidad', p_cantidad,
      'enlazado_a_existente', v_enlazado_a_existente
    )
  );
...
GRANT EXECUTE ON FUNCTION public.normalizar_producto_transferencia(text) TO PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.transferir_tienda_a_terraza(uuid, integer, uuid) TO PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.transferir_terraza_a_tienda(uuid, integer, uuid) TO PUBLIC, anon, authenticated, service_role;
